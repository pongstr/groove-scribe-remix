...
{#if !$ui.practiceMode.active && !$ui.previewMode}
  <div
    ...
    class={cn(
      'no-print bg-background relative bottom-0 left-0 flex max-h-125 w-full min-w-0 flex-col items-stretch justify-start border-t transition-[height,min-height] duration-300 ease-out',
    )}
  >
