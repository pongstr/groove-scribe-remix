...
function moveAccentsOrOtherModifiersOutsideOfGroup(
  ...
  if (rindex > -1) {
    found_modifier = true
    abcNoteStrings.notes2 = abcNoteStrings.notes2.replace(
      modifier_to_look_for,
      '',
    )
  }
  ...
  if (rindex > -1) {
    found_modifier = true
    abcNoteStrings.notes3 = abcNoteStrings.notes3.replace(
      modifier_to_look_for,
      '',
    )
  }
...
function getABCforNote(
  ...
  for (
    var which_array = 0;
    which_array < note_array_of_arrays.length;
    which_array++
  ) {
    if (
      ...
      nextCount = 1
      for (
        var indexA = start_index + 1;
        indexA < start_index + end_of_group;
        indexA++
      ) {
        if (
          !testArrayOfArraysForEquality(note_array_of_arrays, indexA, false)
        ) {
          break
        } else {
          nextCount++
  ...
  if (num_notes_on > 1) {
    ...
    ABC_String +=
      '[' +
      abcNoteStrings.notes1 +
      abcNoteStrings.notes2 +
      abcNoteStrings.notes3 +
      ']' // [^gc]
  } else {
    ABC_String +=
      abcNoteStrings.notes1 + abcNoteStrings.notes2 + abcNoteStrings.notes3 // note this could be a noOp if all strings are blank
  }

  return ABC_String
}

function getABCforRest(
  note_array_of_arrays,
  start_index,
  end_of_group,
  scaler,
  ...
  if (testArrayOfArraysForEquality(note_array_of_arrays, start_index, false)) {
    var restCount = 1
    for (
      var indexB = start_index + 1;
      indexB < start_index + end_of_group;
      indexB++
    ) {
...
function count_active_notes_in_arrays(
  array_of_arrays,
  start_index,
  how_far_to_measure,
) {
  ...
  for (var i = 0; i < num_notes; i++) {
    ...
    if (i % abc_gen_note_grouping_size(true, timeSigTop, timeSigBottom) === 0) {
      ...
      if (0 == count_active_notes_in_arrays(all_drum_array_of_array, i, 12)) {
        ...
        0 == count_active_notes_in_arrays(all_drum_array_of_array, i + 1, 11)
      ) {
        ...
        for (var eighth_index = i; eighth_index <= i + 6; eighth_index += 6) {
          // code duplicated from below
          // clear any invalid stickings since they will mess up the formatting greatly
          for (si = eighth_index + 1; si < eighth_index + 6; si++)
        ...
        for (eighth_index = i; eighth_index <= i + 9; eighth_index += 3) {
          // code duplicated from below
          // clear any invalid stickings since they will mess up the formatting greatly
          for (si = eighth_index + 1; si < eighth_index + 3; si++)
            sticking_array[si] = false
          stickings_voice_string += getABCforRest(
            [sticking_array],
            eighth_index,
        ...
        if (sub_division == 48) {
          ...
          for (var j = i; j < i + 12; j += 4) {
            if (
              0 <
              count_active_notes_in_arrays(all_drum_array_of_array, j + 1, 3)
            ) {
...
export function get_top_ABC_BoilerPlate(
  ...
  // abc2svg rejects `%%stretchlast 0` because it does `if (!parseFloat(param))`.
