...
export function calc_notes_per_measure(
  division,
  time_sig_top,
  time_sig_bottom,
) {
...
export function isTripletDivisionFromNotesPerMeasure(
  notesPerMeasure,
  timeSigTop,
  timeSigBottom,
) {
...
export function notesPerMeasureInFullSizeArray(
  is_triplet_division,
  timeSigTop,
  timeSigBottom,
) {
...
export function getNoteScaler(notes_per_measure, timeSigTop, timeSigBottom) {
  ...
  if (!timeSigTop || timeSigTop < 1 || timeSigTop > 36) {
    ...
    if (
      ...
      scaler = Math.ceil(
        notesPerMeasureInFullSizeArray(true, timeSigTop, timeSigBottom) /
          notes_per_measure,
      )
    else
      scaler = Math.ceil(
        notesPerMeasureInFullSizeArray(false, timeSigTop, timeSigBottom) /
          notes_per_measure,
      )
  }
